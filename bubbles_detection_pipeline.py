...
API_TOKEN_FOR_GD1_5 = "REDACTED_HF_TOKEN"
...
#change the checkpoint for different SAM2 model
...
# Crop each saved frame to keep only the bottom 310 rows (bottom 310x1024 region)
CROP_TOP = 800 - 310  # start row for cropping (490)
for fname in tqdm(os.listdir(source_frames), desc="Cropping Frames"):
    ...
    if os.path.splitext(fname)[-1].lower() in [".jpg", ".jpeg"]:
        ...
        cropped = img[CROP_TOP:, :, :]  # keep bottom 310 rows, full width
